...
final class PersonalChatViewModel: BaseViewModel {
    ...
    func getChatRooms() {
        useCase.getChatRooms(page: page, take: take, isPrivate: true)
    }
    
    func getMoreChatRooms() {
        guard let isLoading = try? isLoading.value(), !isLoading,
              let chatRooms = try? chatRooms.value(), chatRooms.count <= total,
              hasNextPage
        else {
            return
        }
    ...
    func showPersonalChatViewController(chatRoom: ChatRoom) {
        chatMainViewDelegate?.showPersonalChatViewController(chatRoom: chatRoom, nil)
    }
    ...
    private func updateChatRooms(existingRooms: [ChatRoom], newData: [ChatRoom]) -> [ChatRoom] {
        ...
        let updatedExistingRooms = newData.filter { commonRoomIds.contains($0.id) }
